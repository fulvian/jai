...
class IntentMonitoringDashboard:
    ...
    def print_header(self):
        ...
        print("║" + " " * 25 + f"Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}" + " " * 27 + "║")
        print("╚" + "═" * 78 + "╝")
        print()
    ...
    def print_configuration(self):
        ...
        print(f"│ Intent Analysis Timeout:     {str(f'{self.config.intent_analysis_timeout}s'):50} │")
    ...
    def print_intent_metrics(self):
        ...
        print(f"│ Error Rate:                  {str(f'{metrics.get(\"error_rate\", 0):.1%}'):50} │")
        print(f"│ Avg Latency:                 {str(f'{metrics.get(\"avg_latency_ms\", 0):.1f}ms'):50} │")
        print(f"│ Min Latency:                 {str(f'{metrics.get(\"min_latency_ms\", 0):.1f}ms'):50} │")
        print(f"│ Max Latency:                 {str(f'{metrics.get(\"max_latency_ms\", 0):.1f}ms'):50} │")
        print("└" + "─" * 78 + "┘")
        print()
    ...
    def print_accuracy_metrics(self):
        ...
        print(f"│ Weather Queries:             {str(f'{metrics.get(\"weather_accuracy\", 0):.1%}'):50} │")
        print(f"│ Conversational Queries:      {str(f'{metrics.get(\"conversational_accuracy\", 0):.1%}'):50} │")
        print(f"│ Multi-Domain Queries:        {str(f'{metrics.get(\"multi_domain_accuracy\", 0):.1%}'):50} │")
        print(f"│ Overall Accuracy:            {str(f'{metrics.get(\"overall_accuracy\", 0):.1%}'):50} │")
        print("└" + "─" * 78 + "┘")
        print()
    ...
    def print_alerts(self):
        ...
        if metrics.get('avg_latency_ms', 0) > 200:
            alerts.append(f"⚠ High latency: {metrics.get('avg_latency_ms', 0):.1f}ms (target: <200ms)")
        ...
        if metrics.get('weather_accuracy', 0) < 0.95:
            alerts.append(f"⚠ Low weather accuracy: {metrics.get('weather_accuracy', 0):.1%} (target: ≥95%)")
        
        if metrics.get('conversational_accuracy', 0) < 0.98:
            alerts.append(f"⚠ Low conversational accuracy: {metrics.get('conversational_accuracy', 0):.1%} (target: ≥98%)")
    ...
    def print_recommendations(self):
        ...
        if metrics.get('weather_accuracy', 0) < 0.95:
            recommendations.append("→ Review weather query misclassifications and update domain definitions")
