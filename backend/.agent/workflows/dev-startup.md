...
> ⚠️ **ATTENZIONE SESSIONI**: Le sessioni chat sono persistite in **Redis** (container Docker di Me4BrAIn).
> Se riavvii solo Me4BrAIn, il gateway PersAn perde la connessione Redis e le sessioni appaiono
> "cancellate". I dati restano in Redis — **riavvia sempre anche PersAn** dopo Me4BrAIn.
...
Me4BrAIn può essere avviato in **due modalità**:

### Modalità Background (RACCOMANDATA — sopravvive alla chiusura del terminale):
// turbo
```bash
bash /Users/fulvioventura/me4brain/scripts/start.sh --background
```
Log: `tail -f /tmp/me4brain.log`

### Modalità Foreground (per debug, logs a stdout):
// turbo
```bash
...
### Sessioni chat sparite dopo restart
**Causa**: È stato riavviato solo Me4BrAIn senza riavviare PersAn.
Il gateway PersAn si connette a Redis per leggere le sessioni. Se il gateway non viene
riavviato, perde la connessione e mostra zero sessioni. I dati restano tutti in Redis.
**Fix**: Riavviare PersAn:
```bash
bash /Users/fulvioventura/persan/scripts/start.sh
```
**Verifica**: Controllare che Redis contenga le sessioni:
```bash
docker exec me4brain-redis redis-cli KEYS "persan:chat:*" | head -20
```
