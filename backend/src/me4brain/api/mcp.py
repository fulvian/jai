...
@mcp.tool()
async def query_engine(query: str, session_id: Optional[str] = None) -> str:
    """
...
async def nba_betting_analyzer(
    ...
    if home_team: args["home_team"] = home_team
    if away_team: args["away_team"] = away_team
    
    result = await engine.execute_tool("nba_betting_analyzer", args)
    return str(result.data) if hasattr(result, "success") and result.success else f"Error: {getattr(result, 'error', 'Unknown')}"
