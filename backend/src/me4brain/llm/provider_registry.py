...
class ProviderRegistry:
    ...
    def _get_fernet(self, key: str | None):
        if not key:
            import os

            key = os.environ.get("ME4BRAIN_API_KEY")
            if not key:
                logger.error(
                    "provider_registry_missing_api_key",
                    message="ME4BRAIN_API_KEY environment variable not set. "
    ...
    def update(self, provider_id: str, data: dict) -> LLMProviderConfig | None:
    ...
    def update_token_usage(self, provider_id: str, tokens_used: int) -> dict | None:
