...
class ToolCallingEngine:
    ...
    async def create_legacy(
        ...
        """[DEPRECATED] Legacy factory - sends ALL tools to LLM.

        WARNING: This method sends all 129+ tools to the LLM, which may cause
        payload size errors (NanoGPT 40KB limit). Use create() instead.
        ...
        # Update models if using local tool calling
        # Quando use_local_tool_calling=true, usiamo il modello locale per tutto
        if config.use_local_tool_calling:
            routing_model = config.ollama_model  # Usa modello locale per routing
            synthesis_model = config.ollama_model  # Usa modello locale per synthesis
        else:
            if routing_model is None:
                routing_model = config.model_agentic  # Kimi K2.5 for function calling
            if synthesis_model is None:
                synthesis_model = config.model_primary  # Mistral Large for synthesis
        ...
        # Create components
        router = ToolRouter(catalog, llm_client, routing_model)
        executor = ParallelExecutor(
            catalog,
            timeout_seconds=timeout_seconds,
        ...
        if _should_use_unified_intent(config, "engine_create_legacy"):
            await analyzer.warm_up()

        return cls(catalog, router, executor, synthesizer, analyzer=analyzer, config=config)

    @classmethod
    async def create_with_hybrid_routing(
        cls,
        routing_model: str | None = None,
        synthesis_model: str | None = None,
        timeout_seconds: float = 60.0,
        ...
        """[DEPRECATED] Alias for create() - kept for backward compatibility."""
        import warnings

        warnings.warn(
            "create_with_hybrid_routing() is deprecated. Use create() directly.",
            DeprecationWarning,
            stacklevel=2,
        )
        return await cls._create_with_hybrid_routing(
